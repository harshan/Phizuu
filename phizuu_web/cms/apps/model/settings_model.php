...
class SettingsModel {
    ...
    function getFacebookLink($userId){
    ...
    function checkSettings($settings_arr, $userId = NULL) {
        ...
        $sql= "select * from `setting` WHERE type='".addslashes($settings_arr[0]['type'])."' AND value='".addslashes($settings_arr[0]['name'])."' $append";
    ...
    function getRssFeed($type, $appId=NULL) {
        if ($appId == NULL) {
            ...
            $result= mysql_query($sql) or die(mysql_error());
            $this->helper = new Helper();
            $res = $this->helper->_result($result);
            $userId = $res[0]->id;
        }

        $sql= "select * from `setting` WHERE user_id =".$userId." AND type =".$type." ";
    ...
    function saveSettingsFromAPI ($settings) {
        ...
        //echo $settingsStr;
        $settingsStr .= "&appversion={$_POST['AppVersion']}&appname={$_POST['AppName']}&status={$_POST['Status']}&development={$_POST['Development']}&devcertificate={$_POST['DevCertificate']}&prodcertificate={$_POST['ProdCertificate']}";
