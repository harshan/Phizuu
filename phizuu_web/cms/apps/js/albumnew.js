function delete_confirm(id) {
    if( confirm("Are you sure to delete this picture?")){
        ...
        $.post('../../../controller/photo_all_controller.php?action=delete_picture',{
            'id':id
        }, function(data) {
            $('#id_'+id).fadeOut();
            if(flickrListed) {
...
function delete_confirm_album(id) {
    if( confirm("If you delete the album, all the images in it will be moved to the bank list. Are you sure you want to delete this album?")){
        ...
        $.post('../../../controller/photo_all_controller.php?action=delete_album',{
            'id':id
        }, function(data) {
            if(data!='')
...
$(function(){
    ...
    $("#thumbWaitDiv").dialog({
        ...
        open: function(event, ui) {
            $('#thumbWaitDiv').parent().find('a.ui-dialog-titlebar-close').hide();
        },
        show: 'blind',
        autoOpen: false
...
function addImageToAlbum(imageId) {
    ...
    $.post('../../../controller/photo_all_controller.php?action=add_picture_to_album', {
        'image_id':imageId, 
        'album_id': albumId
    } ,function(data) {
...
function removeImageFromAlbum(imageId) {
    ...
    $.post('../../../controller/photo_all_controller.php?action=remove_picture_from_album', {
        'image_id':imageId, 
        'album_id': albumId
    } ,function(data) {
        var refreshAlbum = albumId;
...
function updateIphoneStatus(imageId) {
    var albumId = openedAlbum;
    $.post('../../../controller/photo_all_controller.php?action=updateIphoneStatus', {
        'image_id':imageId
    } ,function(data) {
        var refreshAlbum = albumId;
        var imageCount = parseInt($('#aid_'+refreshAlbum+' .imageCountSel').html());
...
function refreshAlbumThumb(albumId, imageCount) {
    ...
    $.post('../../../controller/photo_all_controller.php?action=refresh_thumb', {
        'album_id': albumId
    } ,function(data) {
...
function openAlbum(albumId) {
    ...
    $.post('../../../controller/photo_all_controller.php?action=open_album', {
        'album_id': albumId
    }, function(data) {
...
function showEditAlbum(albumId) {
    ...
    if (openedAlbum == -1) {
        ...
        $.post('../../../controller/photo_all_controller.php?action=get_album_details_ajax', {
            'album_id':albumId
        } ,function(data) {
            $('#album_edit_save_button').removeClass('ui-state-disabled');
            $('#album_edit_save_button').attr("disabled", false);
...
function flickrGetCollections() {
    ...
    $.post('../../../controller/photo_all_controller.php?action=list_flickr_pics', {
        'username':username
    }, function(data) {
...
function flickrGetImages() {
    ...
    if(selectedAlbum != 0){
        ...
        $('#list_4').empty();
        $.post('../../../controller/photo_all_controller.php?action=get_photos_album_facebook', {
            'id':id
        }, function(data) {
            // alert(data);
            ...
            $('#flickrButton').show();
            $('#listingWaiting').hide();
            $('#flickrUserName').removeAttr("disabled");
            $('#flickrCollection').removeAttr("disabled");
            $('#addAllButtonLnk').show();
            $(data.html).appendTo('#list_4');
            flickrListed = true;
        },'json');
...
function addItemFlickr(clicked, added) {
    ...
    $.post('../../../controller/photo_all_controller.php?action=add_pic_album_1', {
        'name':name,
        'uri':url,
        'thumb_uri':thumb,
        'pid':pid
        ...
        //alert(data);
        li.fadeOut(300);
        $(data).appendTo('#list_1').fadeIn(500);
    });
}
...
function showSelectImageList() {
    ...
    $.post('../../../controller/photo_all_controller.php?action=get_select_pic_list', {
        'editing_album':editingAlbum
    }, function(data) {
        $('#selectImage').hide().fadeOut(300, function(){
            $('#selectImage').html(data).fadeIn(300);
        });
    });
...
function selectPicture(itemElem) {
    ...
    $.post('../../../controller/photo_all_controller.php?action=upload_image_from_web', {
        'image_url':item.children('.selURL').html()
        }, function(data) {
...
function listFanPhotos() {
    ...
    $.post('../../../controller/photo_all_controller.php?action=get_photos_album_fan_photos', {
        'id':'x'
    }, function(data) {
